...
if __name__=="__main__":
    archs = ['cResNet18',  'cResNet34', 'cResNet50', 'cResNet101']
    n_bins = 10
    ...
    for arch in archs:
        ...
        bins = np.linspace(0, 1, 10)
        noise_mean_acc, noise_sd_acc, noise_mean_ece, noise_sd_ece, noise_ensem_acc, noise_ensem_ece = [], [], [], [], [], []
        noises = ['brightness', 'pixelate', 'contrast', 'defocus_blur', 'elastic_transform', 'fog', 'frost', 'gaussian_blur', 'gaussian_noise', 'glass_blur', 'impulse_noise', 'jpeg_compression', 'motion_blur', 'saturate', 'snow', 'spatter', 'speckle_noise', 'zoom_blur']
        for noise in noises:
            runs = [1, 2, 3, 4, 5]
            run_outputs = []
            run_acc = []
            ...
            run_alphas = []
            for run in runs:
                in_outputs = np.load("outputs/output_"+arch+'_CIFAR10_'+spar+'_'+noise+'_'+str(run)+'.npy')
                gts = np.load('outputs/gts_'+arch+'_CIFAR10_'+spar+'_'+noise+'_'+str(run)+'.npy')
                pred = in_outputs.argmax(axis = 1)
                err = sum(np.neq(pred, gts).astype(int))/len(gts)
            ...
            noise_mean_acc.append(mean_acc)
            noise_sd_acc.append(sd_acc)

            mean_ece, sd_ece = np.mean(run_ece), np.std(run_ece)
            noise_mean_ece.append(mean_ece)
            noise_sd_ece.append(sd_ece)

            run_alphas = np.array(run_alpha])
            run_alphas = run_alphas/sum(run_alphas)
            ...
            noise_ensem_acc.append(acc)
            noise_ensem_ece.append(ece)
        noise_ensem_ece = np.array(noise_ensem_ece)
        noise_ensem_acc = np.array(noise_ensem_acc)
        noise_mean_acc = np.array(noise_mean_acc)
        noise_mean_ece = np.array(noise_mean_ece)
        print("For sparsity:", spar, "ensemble accuracy:", np.mean(noise_ensem_acc), "ece", np.mean(noise_ensem_ece), "run accuracy:", np.mean(noise_mean_acc), "ece", np.mean(noise_mean_ece))
