...
class CustomDataCIFAR10:
    def __init__(self, data_root, seed, transform= None, train = True):
        super(CustomDataCIFAR10, self).__init__()
        if train:
...
class CIFAR10:
    def __init__(self, args):
        ...
        train_dataset = CustomDataCIFAR10(data_root,  args.seed, transform = transform, train = True)
        if args.seed==1:
            sampler = None 
            shuffle = True
        else:
        ...
        test_dataset = CustomDataCIFAR10(data_root,  args.seed, transform = transform, train = False)
        self.train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=args.batch_size, sampler = sampler, shuffle=shuffle, **kwargs
        )
        train_infer_dataset = CustomDataCIFAR10(data_root,  args.seed, transform = transform, train = True)
